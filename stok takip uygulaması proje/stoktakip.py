...
# Veritabanını belleğe yükleyen fonksiyon
def veritabani_yukle():
    conn = sqlite3.connect("stok_takip.db")
    cursor = conn.cursor()
    cursor.execute("SELECT urun_adi, stok_miktari FROM stok")
    rows = cursor.fetchall()
    for urun_adi, stok_miktari in rows:
        stok[urun_adi] = stok_miktari
        if urun_adi not in urun_sirasi:
...
# Uygulama başlatılırken veritabanını yükle
veritabani_baglanti()  # Veritabanını başlat
veritabani_yukle()     # Veritabanındaki stokları belleğe yükle
